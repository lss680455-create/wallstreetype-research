...
| H1 | pipeline_orchestration.md: 8 stages, chained start-to-finish | PASS | S1 Input&Envelope → S2 Master Brief → S3 Parallel Research → S4 Red Team → S5 Editorial Adjudication → S6 Charting → S7 Layout → S8 Final Review & Delivery; each has Who/Input/Output/quality gate (G0..G6) with fail-loops; S1 output (envelope.json) consumed by S2 … S7 output consumed by S8 | none |
